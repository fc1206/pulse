...
1. **Never hand-edit** `data/registry.csv`, `data/SCANLOG.md`, `data/state.json`, `data/DIGEST.md`, `data/ACTIONS.md`, or the changelog in `data/LANDSCAPE.md`. The only writers are `scripts/validate_merge.py` (registry/scanlog/state/changelog) and `scripts/validate_digest.py` (digest + action ledger — sole writer of `data/ACTIONS.md`). You write `runs/<date>/*` files; the scripts do the rest.
...
`runs/<date>/digest.md` — markdown per `config/digest-spec.md` (0–2 cited, actionable items derived against `config/context.md`, or the NO ACTIONABLE SIGNAL sentinel). Validated and prepended to `data/DIGEST.md` by `scripts/validate_digest.py`, which also logs each item's Action line to `data/ACTIONS.md`.
...
- `config/axes.json` — the competitive map's axes + scoring signals (owner-editable)
- `data/registry.csv` — system of record, append-mostly
- `data/LANDSCAPE.md` — narrative map + changelog
- `.claude/commands/onboard.md` — one-command first-run onboarding (calls /setup, whose final step is the deep-map first scan; GUI-only key step)
