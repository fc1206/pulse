...
- **`config/queries.md`** — filling `{competitor}`/`{category}`/`{your-product}` placeholders is NOT enough: **rewrite each block's subject wholesale for their market** (retitle blocks whose vocabulary isn't theirs — Block B especially ships as an example shape, not a fill-in), and swap Block I's directory sources for the ones their market actually lives on (Clutch/Behance for agencies and studios, G2/Capterra for SaaS). Keep all blocks (A–I) and the two-lane structure; Block F (always-on), H (regional), and I (edge-expansion) are the recall safety net — never trim them. **Block F must keep BOTH an "alternatives" query AND a "vs" query** (e.g. `"{category} alternatives"` *and* `"vs {competitor}"`) — pytest asserts each channel separately, so rewrite the *subject*, never delete either. Leave the tuning log empty at config time; the seed sweep appends its own first dated lesson afterward (it IS the first scan).
...
2. Run it. **If the surface supports subagents / parallel tasks,** fan out one researcher per query block in parallel and merge their candidates (dedupe by domain) before validation. **If it doesn't,** run the blocks sequentially and say so honestly — never quietly skip blocks to hit an estimate. Fold the known competitors from Q3 into the sweep; verify those first, they anchor the rubric's calibration. **Sweep every cluster in `config/clusters.json`, not just direct lookalikes** — incumbents whose suites bundle this, platforms/infra rivals build on, adjacent players the buyer confuses them with. On the radar this template came from, direct rivals were barely a third of the day-one map. Harvest the category's *real* directories (Block I — Clutch/Behance for agencies and studios, G2/Capterra for SaaS); that's where the established players your keyword vocabulary misses are listed. If a cluster ends the sweep empty, name it and say why — never silently skip it.
3. **Acceptance standard — wider aperture, same bar, never a lower one:**
   - **Live evidence URL mandatory** (web fetch/search) — no URL, no entry; unverifiable-but-plausible → `watch-unconfirmed` in run notes. If the environment blocks direct page fetches, verification through search-result snippets satisfies the bar — note it in `run_meta.json`.
