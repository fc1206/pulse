...
- **`config/clusters.json`** — write the same cluster names here. This is the single source the merge validates against, so the radar is retargeted by editing **config only — never any Python or test**. Then set matching `x_cluster_base`/`y_cluster_base` keys in `config/axes.json` so the map scores the new clusters.
...
- **`config/brand.json`** — set `product` to the radar's name (e.g. `"<Company> Radar"`) — this drives the report's eyebrow and browser-tab title, so leaving the default makes a fork read "Pulse" while the body shows their brand. Set `company` to their company name, `accent` (and `accent_2` if they gave one), `logo` (emoji or image URL, if any), and `theme` (`light` default, or `dark`). This brands the report, digest, and emails — the first thing they see.
